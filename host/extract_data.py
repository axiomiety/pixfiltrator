...
def weigh_blocks(vals, width, sum_values=False):
    ...
    ret = []
    for val in vals:
        res = np.multiply(np.array(val).reshape((width, width)), p).reshape(
            1, num_elems
        )
        if sum_values:
...
def extract(image, sqWidth, regionOnGuest, outFile, verify):
    ...
    if outFile is None:
        ...
        outFile = os.path.join(
            bp,
            f'block_{str(meta_data.page_num).zfill(5)}_{str(meta_data.num_pages).zfill(5)}_{meta_data.sha1.decode("ascii")}',
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--out', action='store', default='out.bin', help='out path for the decoded file'
    )
    parser.add_argument(
        '--verify',
        action='store_true',
